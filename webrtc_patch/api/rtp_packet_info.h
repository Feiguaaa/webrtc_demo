...
class RTC_EXPORT RtpPacketInfo {
 public:
  ...
  std::optional<uint16_t> encoder_target_bitrate_kbps() const {
    return encoder_target_bitrate_kbps_;
  }
  RtpPacketInfo& set_encoder_target_bitrate_kbps(uint16_t value) {
    encoder_target_bitrate_kbps_ = value;
    return *this;
  }

  // Frame packet info from RTP header extension for precise loss detection.
  std::optional<uint16_t> total_frame_packets() const {
    return total_frame_packets_;
  }
  RtpPacketInfo& set_total_frame_packets(uint16_t value) {
    total_frame_packets_ = value;
    return *this;
  }
  std::optional<uint16_t> frame_packet_index() const {
    return frame_packet_index_;
  }
  RtpPacketInfo& set_frame_packet_index(uint16_t value) {
    frame_packet_index_ = value;
    return *this;
  }
  std::optional<uint16_t> frame_sequence() const {
    return frame_sequence_;
  }
  RtpPacketInfo& set_frame_sequence(uint16_t value) {
 ...
 private:
  ...
  // Encoder target bitrate from RTP header extension (kbps).
  std::optional<uint16_t> encoder_target_bitrate_kbps_;

  // Frame packet info from RTP header extension for precise loss detection.
  std::optional<uint16_t> total_frame_packets_;
  std::optional<uint16_t> frame_packet_index_;
  std::optional<uint16_t> frame_sequence_;
};
