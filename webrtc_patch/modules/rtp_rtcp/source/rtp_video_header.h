...
// Carries frame packet info for precise receiver-side loss detection.
// total_packets: total number of RTP packets in this frame (max 1023).
// packet_index: 0-based index of this packet within the frame (max 63).
// frame_sequence: monotonically increasing frame counter (max 1023, wraps).
...
struct RTPVideoHeader {
  ...
  // Encoder target bitrate in kbps, carried via custom RTP header extension.
  std::optional<uint16_t> encoder_target_bitrate_kbps;

  // Frame packet info (total_packets, packet_index), carried via custom RTP
  // header extension on every packet for precise receiver-side loss detection.
  std::optional<FramePacketInfo> frame_packet_info;
};
