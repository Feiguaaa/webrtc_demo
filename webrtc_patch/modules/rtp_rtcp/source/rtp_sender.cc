...
bool IsNonVolatile(RTPExtensionType type) {
  switch (type) {
    ...
    case kRtpExtensionFramePacketInfo:
      return true;
    ...
    case kRtpExtensionEncoderTargetBitrate:
      return false;
...
std::unique_ptr<RtpPacketToSend> RTPSender::AllocatePacket(
  ...
  // Reserve space for frame packet info extension (used for precise loss detection).
  packet->ReserveExtension<FramePacketInfoExtension>();
