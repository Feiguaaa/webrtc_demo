...
void RTPSenderVideo::AddRtpHeaderExtensions(const RTPVideoHeader& video_header,
  ...
  if (first_packet && video_header.encoder_target_bitrate_kbps) {
    packet->SetExtension<EncoderTargetBitrateExtension>(
        *video_header.encoder_target_bitrate_kbps);
  }
...
bool RTPSenderVideo::SendVideo(int payload_type,
  ...
  for (size_t i = 0; i < num_packets; ++i) {
    ...
    // Set frame packet info extension on every packet for precise loss
    ...
    if (num_packets <= 1023) {
      ...
      static int ext_warn = 0;
      if (!ok && ++ext_warn == 1) {
        fprintf(stderr, "[Sender] WARN: SetExtension<FramePacketInfo> FAILED for frame seq=%u pkts=%zu\n",
                frame_sequence_counter_, num_packets);
      }
    }
  ...
  // Increment frame sequence counter (wraps at 1024).
  frame_sequence_counter_ = (frame_sequence_counter_ + 1) & 0x3FF;
