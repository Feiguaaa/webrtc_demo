...
// Carries encoder target bitrate in kbps (uint16, big-endian).
// Only written on the first packet of each frame.
class EncoderTargetBitrateExtension {
 public:
  using value_type = uint16_t;
  static constexpr RTPExtensionType kId = kRtpExtensionEncoderTargetBitrate;
  static constexpr uint8_t kValueSizeBytes = 2;
  static constexpr absl::string_view Uri() {
    return RtpExtension::kEncoderTargetBitrateUri;
  }

  static bool Parse(std::span<const uint8_t> data, uint16_t* bitrate_kbps) {
    if (data.size() < kValueSizeBytes) return false;
    *bitrate_kbps = ByteReader<uint16_t>::ReadBigEndian(data.data());
    return true;
  }
  static size_t ValueSize(uint16_t /* bitrate_kbps */) {
    return kValueSizeBytes;
  }
  static bool Write(std::span<uint8_t> data, uint16_t bitrate_kbps) {
    if (data.size() < kValueSizeBytes) return false;
    ByteWriter<uint16_t>::WriteBigEndian(data.data(), bitrate_kbps);
    return true;
  }
};

// Carries frame packet info: total_packets (10 bits), packet_index (6 bits),
// and frame_sequence (10 bits, wraps at 1024) for detecting completely lost frames.
// Written on every packet of a frame. Allows receiver to know exactly how many
// packets the sender put into each frame and track frame sequence gaps.
class FramePacketInfoExtension {
 public:
  using value_type = FramePacketInfo;
  static constexpr RTPExtensionType kId = kRtpExtensionFramePacketInfo;
  static constexpr uint8_t kValueSizeBytes = 4;
  static constexpr absl::string_view Uri() {
    return RtpExtension::kFramePacketInfoUri;
  }
  ...
  static bool Write(std::span<uint8_t> data, const FramePacketInfo& info) {
    if (data.size() < kValueSizeBytes) return false;
