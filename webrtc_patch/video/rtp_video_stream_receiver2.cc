...
std::atomic<PerFrameLossTracker*> g_per_frame_loss_tracker{nullptr};
...
bool RtpVideoStreamReceiver2::OnReceivedPayloadData(
  ...
  // Read encoder target bitrate from RTP header extension (first packet only).
  uint16_t bitrate_kbps = 0;
  if (rtp_packet.GetExtension<EncoderTargetBitrateExtension>(&bitrate_kbps)) {
    packet_info.set_encoder_target_bitrate_kbps(bitrate_kbps);
  }
  // Read frame packet info extension for precise loss detection.
  FramePacketInfo frame_info;
  if (rtp_packet.GetExtension<FramePacketInfoExtension>(&frame_info)) {
    packet_info.set_total_frame_packets(frame_info.total_packets);
    packet_info.set_frame_packet_index(frame_info.packet_index);
    packet_info.set_frame_sequence(frame_info.frame_sequence);
  }
  // Set sequence number for loss tracking.
  packet_info.set_sequence_number(rtp_packet.SequenceNumber());
...
void RtpVideoStreamReceiver2::OnRtpPacket(const RtpPacketReceived& packet) {
  ...
  // Per-frame loss tracking at RTP layer (records ALL frames, including incomplete).
  // Uses global pointer set by receiver main.cc.
  // Encoder target bitrate is extracted only from the first packet of each frame
