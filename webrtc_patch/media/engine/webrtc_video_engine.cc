...
WebRtcVideoEngine::GetRtpHeaderExtensions(
  ...
  id = RtpHeaderExtensionId(id.value() + 1);
  // Encoder target bitrate: sent by sender, read by receiver.
  result.emplace_back(RtpExtension::kEncoderTargetBitrateUri, id,
                      RtpTransceiverDirection::kSendRecv);
  id = RtpHeaderExtensionId(id.value() + 1);
  // Frame packet info: sent by sender on every packet, read by receiver
  // for precise per-frame loss detection.
  result.emplace_back(RtpExtension::kFramePacketInfoUri, id,
                      RtpTransceiverDirection::kSendRecv);
  id = RtpHeaderExtensionId(id.value() + 1);
...
void WebRtcVideoSendChannel::OnReadyToSend(bool ready) {
  ...
  fprintf(stderr, "[VideoChannel] OnReadyToSend: ready=%d\n", ready);
...
WebRtcVideoSendChannel::WebRtcVideoSendStream::WebRtcVideoSendStream(
    ...
    : send_channel_(send_channel),
      env_(env),
      thread_checker_(call->worker_thread()),
      ssrcs_(sp.ssrcs),
...
void WebRtcVideoReceiveChannel::SetDefaultSink(
  ...
  default_sink_ = sink;
  if (std::optional<uint32_t> ssrc = GetUnsignaledSsrc()) {
